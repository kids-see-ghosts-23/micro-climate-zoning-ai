...
export default function Sidebar({ onSubmit, loading, jobStatus, error, results, selectedBlock, onSelectBlock }) {
  ...
  const handleRun = async () => {
    ...
    // If a preset is selected and city input matches, use preset coords directly
    if (resolvedCity) {
      ...
      {tab === 'config' && (
        <div style={{ padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 18, overflow: 'auto' }}>
          ...
          <div>
            ...
            {resolvedCity && (
              <div style={{ fontSize: 10, color: '#10b981', marginTop: 5, opacity: 0.8 }}>
                Located: {resolvedCity.name}
              </div>
            )}
            {geocodeError && (
              <div style={{ fontSize: 10, color: '#f87171', marginTop: 5 }}>
                {geocodeError} — try a preset or check spelling
              </div>
            )}
          </div>
              ...
              <div style={{ padding: '10px 12px' }}>
                ...
                {selectedBlockData.directives?.map((d, i) => (
                  <div key={i} style={{
                    ...
                    background: `${dc(d.code)}08`, border: `1px solid ${dc(d.code)}25`,
                  }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {block.directives?.slice(0, 4).map((d, i) => (
                    <span key={i} style={{
                      ...
                      background: `${dc(d.code)}12`, color: dc(d.code),
